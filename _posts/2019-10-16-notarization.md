...
``--deep``オプションは，すでに署名されているリソースの署名は上書きしません。コードコード署名は，原則的にフォルダー構造の中から外に向かって実施します。4Dの場合，プリインストールされたアプリや実行ファイルにはエンタイトルメントとHardened Runtimeオプションを指定する必要があります。プラグイン・フレームワーク・ライブラリなどのリソースは，エンタイトルメントを指定せずにコード署名を実施します。最後に外殻のアプリ本体を署名しますが，このとき``--force``（強制的に署名）オプションを指定してしまうと，内部リソースの署名が無効になってしまうので，指定はしないでください。
